...
TIME_SAT=$( tail -n 1 sat_result.txt )
sed -i '$ d' sat_result.txt
...
do
  ...
  if [[ "$out" == *"UNSATISFIABLE"* ]]; then
    ...
    { time ./kissat -q sat.cnf; } > sat_result.txt 2>&1
    out=$(head -n 1 sat_result.txt)
    TIME_SAT=$( bc <<<"($TIME_SAT + $( tail -n 1 sat_result.txt ))" )
    sed -i '$ d' sat_result.txt
    ...
    TMP=$( { time ../build/fsa_red --sat_compare -N $N -S $ALPHA "$AUTO_VTF"; } 2>&1)
    out=${TMP::-5}
      ...
      if [[ "$out" == *"PASS"* ]]; then
        ...
        echo "Solver time:                  "$TIME_SAT
          ...
          if [ ! -z "${PRO[*]}" ] && [ ! -z "${CON[*]}" ]; then
            EX=("${EX[@]}" "${PRO[@]}")
            CONEX=("${CONEX[@]}" "${CON[@]}")
            { time ../build/fsa_red --sat_add_clauses -N $N -S $ALPHA -M $MAX -A "${PRO[@]}" -R "${CON[@]}"; } >> sat.cnf 2>&1
          elif [ ! -z "${PRO[*]}" ]; then
            EX=("${EX[@]}" "${PRO[@]}")
            { time ../build/fsa_red --sat_add_clauses -N $N -S $ALPHA -M $MAX -A "${PRO[@]}"; } >> sat.cnf 2>&1
          else [ ! -z "${CON[*]}" ]
            CONEX=("${CONEX[@]}" "${CON[@]}")
            { time ../build/fsa_red --sat_add_clauses -N $N -S $ALPHA -M $MAX -R "${CON[@]}"; } >> sat.cnf 2>&1
          fi
          TIME_GEN=$( bc <<<"($TIME_GEN + $( tail -n 1 sat.cnf ))" )
          sed -i '$ d' sat.cnf
