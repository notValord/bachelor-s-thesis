...
do
  ...
  if [[ "$out" == *"UNSATISFIABLE"* ]]; then
    ...
    TMP=$( { time ../build/fsa_red --sat_nfa_compare -N $N -S $ALPHA "$AUTO_VTF"; } 2>&1)
    out=${TMP:: -5}
      ...
      if [[ "$out" == *"PASS"* ]]; then
        ...
        echo "Solver time:                  "$TIME_SAT
          ...
          if [ ! -z "${PRO[*]}" ] && [ ! -z "${CON[*]}" ]; then
            EX=("${EX[@]}" "${PRO[@]}")
            CONEX=("${CONEX[@]}" "${CON[@]}")
            { time ../build/fsa_red --sat_nfa_add_clauses -N $N -S $ALPHA -M $MAX -A "${PRO[@]}" -R "${CON[@]}"; } >> nfa_sat.cnf 2>&1
          elif [ ! -z "${PRO[*]}" ]; then
            EX=("${EX[@]}" "${PRO[@]}")
            { time ../build/fsa_red --sat_nfa_add_clauses -N $N -S $ALPHA -M $MAX -A "${PRO[@]}"; } >> nfa_sat.cnf 2>&1
          else [ ! -z "${CON[*]}" ]
            CONEX=("${CONEX[@]}" "${CON[@]}")
            { time ../build/fsa_red --sat_nfa_add_clauses -N $N -S $ALPHA -M $MAX -R "${CON[@]}"; } >> nfa_sat.cnf 2>&1
          fi
          TIME_GEN=$( bc <<<"($TIME_GEN + $( tail -n 1 nfa_sat.cnf ))" )
          sed -i '$ d' nfa_sat.cnf
